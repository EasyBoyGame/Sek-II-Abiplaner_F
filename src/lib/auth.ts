...
if (browser) {
  const config: UserManagerSettings = {
    authority: '/realms/abiplaner',
    client_id: 'abiplaner-frontend',
    stateStore: new WebStorageStateStore({ store: window.localStorage }),
    loadUserInfo: true,
    redirect_uri: `${window.location.origin}/auth-callbacks/callback`,
    post_logout_redirect_uri: 'https://abiplaner.catalkaya.dev',
...
export { login, logout, handleCallback, handleSilentCallback, loginSilent, getUsername, getRoles };
